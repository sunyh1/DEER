...
class VisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        scale = width ** -0.5
        self.class_embedding = nn.Parameter(scale * torch.randn(2, width))
        ...
        self.proj = nn.Parameter(scale * torch.randn(2, pool_dim, output_dim))
    ...
    def _embeds(self, x:torch.Tensor, modality_ids:torch.Tensor) -> torch.Tensor:
        x = self.conv1(x)  # shape = [*, dim, grid, grid]
        x = x.reshape(x.shape[0], x.shape[1], -1)  # shape = [*, width, grid ** 2]
        x = x.permute(0, 2, 1)  # shape = [*, grid ** 2, width]

        # class embeddings and positional embeddings
        cls = self.class_embedding[modality_ids]   # shape [B, width]
        cls = cls.unsqueeze(1)                     # [B, 1, width]
    ...
    def forward(self, x: torch.Tensor, modality_ids: torch.Tensor = 0):
        x = self._embeds(x, modality_ids)
        x = self.transformer(x)
        pooled, tokens = self._pool(x)

        if self.proj is not None:
            proj = self.proj[modality_ids]        # [B, pool_dim, output_dim]
            pooled = torch.bmm(pooled.unsqueeze(1), proj).squeeze(1)  # [B, output_dim]
