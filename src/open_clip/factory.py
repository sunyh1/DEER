...
def load_checkpoint(
    ...
    # class_embedding: [width] → [M, width]
    M = 2
    if "visual.class_embedding" in state_dict:
        v = state_dict["visual.class_embedding"]
        if v.dim() == 1 and model.visual.class_embedding.ndim == 2:
            state_dict["visual.class_embedding"] = v.unsqueeze(0).repeat(M, 1)

    # proj: [d, d] → [M, d, d]
    if "visual.proj" in state_dict:
        v = state_dict["visual.proj"]
        if v.dim() == 2 and model.visual.proj.ndim == 3:
            state_dict["visual.proj"] = v.unsqueeze(0).repeat(M, 1, 1)
